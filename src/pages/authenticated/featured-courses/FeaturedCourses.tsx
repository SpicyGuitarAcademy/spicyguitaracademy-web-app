...
export const FeaturedCourses: React.FC<{}> = () => {
  ...
  const { featuredCourses, sortedBy, sortCoursesByOrder, sortCoursesByTitle, sortCoursesByTutor } = useCourseStore()
    ...
    <DashboardWrapper>
      <div className="__studying_course_page">
        ...
        <div className="__greetings mb-3 d-flex justify-content-between align-items-center">
          <h2 className="fw-bold">Buy Courses</h2>
          <div className="input-group w-40 w-md-20">
            ...
            <select onChange={handleSort} value={sortedBy}
              className="bg-primary text-light border-0 form-control w-md-20">
          ...
          <div className="__courses d-flex flex-wrap">
            {
              featuredCourses?.length! > 0 ?
                featuredCourses?.map((item, index) => (
                  <CourseItem clickable={true} isBought={false} isFeatured={true} showProgress={false} item={item} key={index} />
                ))
                : <div>No Courses</div>
            }
          </div>
